...
local function StartFly()
    ...
    Notify("Fly", "Flying at speed "..Config.FlySpeed.."  |  fly again to land", "success")
end

-- fly / fly <speed> — toggles fly or changes speed mid-flight
Reg("fly", {"f"}, "Toggle fly  |  fly 80 to set speed while flying", false, function(a)
...
local function StartNoclip()
    ...
    Notify("Noclip", "Phase through walls  |  noclip again to stop", "success")
end

Reg("noclip", {"nc"}, "Toggle noclip", false, function()
    if ncOn then StopNoclip() else StartNoclip() end
end)
...
local function BuildESPFor(player)
    ...
    N("TextLabel", {
        ...
        Font = Enum.Font.GothamBold, Text = player.DisplayName,
        TextColor3 = Color3.fromRGB(215,165,255), TextSize = 20,
    ...
    while true do
        ...
        for _, p in ipairs(Players:GetPlayers()) do
            ...
            if not c or not root then
                RemoveESPFor(p)
            elseif not espData[p] or not espData[p].hl or not espData[p].hl.Parent or espData[p].hl.Adornee ~= c then
                task.spawn(BuildESPFor, p)
            end
        end
        for p in pairs(espData) do
            if not p or not p.Parent then RemoveESPFor(p) end
        end
    end
end)
...
Players.PlayerAdded:Connect(function(p)
    p.CharacterAdded:Connect(function()
        task.wait(1.2)
        if espOn then task.spawn(BuildESPFor, p) end
    end)
end)
...
Reg("esp", {"e"}, "Toggle ESP", false, function()
    if espOn then
        espOn = false
        ...
        Notify("ESP", "Tracking "..tostring(#Players:GetPlayers()-1).." player(s)", "success")
    end
end)
